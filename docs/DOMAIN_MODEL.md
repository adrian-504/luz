...
Implemented in Phase 1 in `shared/domain` (package `app.iptvplayer.domain`, Kotlin `commonMain`). Field names
below are logical; the Kotlin sources are the exact types. Where implementation refined this document, the change
is recorded here.
...
Types: `Instant` = `kotlin.time.Instant` (stable in Kotlin 2.4); `Lang` = BCP-47 tag; `?` = optional; `*Id` = stable ID (§4).
...
| contentSelection | {live, movies, series, epg: Boolean} | user may skip VOD or EPG import **(refinement)** |
...
Test vectors: `tooling/fixtures/ids/id-vectors.json`, generated by an independent Python reference implementation
(`tooling/scripts/generate_id_vectors.py`) and reproduced exactly by the Kotlin tests. SHA-256 is a pure-Kotlin
implementation so every target hashes identically without platform crypto.

### 4.3 Natural keys

`normKey(s)` = Unicode NFKC → lowercase (invariant) → split on whitespace → join with single spaces.
It does **not** remove quality tags (HD/FHD/4K) — those are distinct channels.

**Whitespace** is an explicit set so all platforms agree regardless of their `isWhitespace` definition: Unicode
categories Zs, Zl, Zp plus U+0009–U+000D, U+001C–U+001F and U+0085.

| Entity | Scope | Natural key |
|---|---|---|
| Channel (Xtream) | playlistId | `"xtream"`, stream_id |
| ChannelGroup (Xtream) | playlistId | contentKind, `"xtream"`, category_id |
...
| Movie (Xtream) / Series (Xtream) | playlistId | `"xtream"`, stream_id / series_id |
| Movie (M3U) | playlistId | normKey(name), normKey(tvg-id) or `""`, collision ordinal |
| Series (M3U) | playlistId | `"m3u"`, normKey(series title parsed from `Show S01E02`) |
| Episode (M3U) | seriesId | `"m3u"`, season, episode, collision ordinal |
| Season | seriesId | season number |
| Episode (Xtream) | seriesId | `"xtream"`, episode id |
| MediaSource | owner id | M3U: `"m3u"`, credential-free URL template; Xtream: `"xtream"`, kind, stream id |
| Program | epgSourceId | XMLTV channel id, start instant (UTC epoch seconds) |
...
1. same credential-stripped URL fingerprint (`StableIds.fingerprint(urlTemplate)`, so opaque tokens are never stored in hints) → 2. same tvg-id and same match-normalized name → 3. unique match-normalized name within the playlist.
...
`equivalenceKey = matchNormalize(name)` + tvg-id when present. Used only for presentation grouping and EPG
matching; never as a primary key. `matchNormalize(s)` is defined exactly as:

1. `k = normKey(s)`.
2. Strip a country prefix: `|xx|` or `|xxx|`, or `xx:` / `xxx:` / `xx|` / `xxx|` at the start, where `x` is `a–z`; trim.
3. Remove bracketed spans `(…)`, `[…]`, `{…}` (first matching closer; unmatched openers stay).
4. Replace punctuation (P\*), symbols (S\*) and every supplementary-plane code point with a space.
5. Split on whitespace and drop quality tokens: `sd hd fhd uhd qhd 4k 8k hdr hevc h264 h265 720p 1080p 1080i 2160p`.
6. If nothing remains, return `normKey(s)` with step 4 applied instead (so `"UK: HD"` → `"uk hd"`, not empty).
...
`ImportDiagnostic(unit?, severity, code, location {line | jsonPath | xmlPath}, message, sampleRedacted)` — `unit` is
null for source-level findings made before classification (M3U parsing);
