...
| Project location outside cloud sync | Reliable builds, intact `.git` | **Resolved 2026-09-14:** repository moved from iCloud-synced `~/Desktop` (where iCloud created `… 2.class` conflict copies inside `build/`) to `~/Developer/IPTV App`, which is not synced; moved again on 2026-09-15 to the external SSD (`/Volumes/DevSSD/IPTV App`, APFS, next to the Android SDK), with the Gradle user home on the SSD too (`~/.gradle` is a link to `/Volumes/DevSSD/gradle-home`). Keep repositories out of iCloud Drive; the SSD must be connected to build |
